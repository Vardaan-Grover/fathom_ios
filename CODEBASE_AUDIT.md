...
**Phase 5 — done (2026-07-08):**
- ✅ Dead code deleted (~1,600 lines): local-LLM pipeline (PreprocessingLLMClient, ChunkBuilder, EntityReconciler, EntitySanitizer, EventSanitizer), vestigial reader chain (ReaderService, ReaderViewModel, AIClient/ContextEngine protocols + impls, AIChatService, `openBook`, Passage/ContextBundle/Explanation models), fully-commented EPUBReaderView, JSONBookRepository, stray `Sources/GRDBTest` SPM experiment + its `.build/` cache. AppContainer/LibraryViewModel signatures shrank accordingly. Empty DB tables (entities, scenes, events…) intentionally left — dropping them buys nothing.
- ✅ **FathomTests target added** (Swift Testing, TEST_HOST = Fathom.app, wired into the shared scheme): 20 tests / 4 suites, all passing — MigrationTests (full chain on in-memory DB, v26 AI-trigger drop, v27 millisecond CDC timestamps, re-queue-during-push survival, delete queuing), CKRecordRoundTripTests (every synced field incl. the Book completion-field regression), StableHashTests (pinned FNV-1a vectors), ParagraphIndexerTests (normalization, junk filter, dedup, index continuity). `DatabaseManager.makeMigrator` is now internal for testability.
- ✅ SwiftLint fully wired: `.swiftlint.yml` (correctness rules on, legacy-style rules deferred to a formatting pass) + build phase on the app target (Homebrew-path aware) + `ENABLE_USER_SCRIPT_SANDBOXING = NO`. Baseline: 906 violations → 0 errors / 8 warnings after config, code fixes (uppercase view-builder names, snake_case DTOs → CodingKeys), and `swiftlint --fix`.
- ✅ 🔑 **Leaked Gemini API key removed** from the shared scheme (`GEMINI_API_KEY` in Fathom.xcscheme, git-tracked) and **revoked in Google AI Studio on 2026-07-21** — no action outstanding. The key value is still reachable in git history (added around `409364c`, removed in `df95d9a`), and this repo is public, so it was exposed for the period between the first push and revocation. That is harmless now the key is dead: revocation is the fix, and rewriting history would not undo the exposure anyway, since GitHub caches unreachable objects and forks keep their own copies. Nothing in the tree references the key — no Swift code calls Gemini, and the shared scheme carries no environment variables.
