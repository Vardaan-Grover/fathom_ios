...
let sceneric: [Photo] = [
    .init(assetName: "GradientTop", author: "John Doe"),
    .init(assetName: "GradientTop", author: "John Doe"),
    .init(assetName: "GradientTop", author: "John Doe"),
...
let illustrations: [Photo] = [
    .init(assetName: "GradientBottom", author: "John Doe"),
    .init(assetName: "GradientBottom", author: "John Doe"),
    .init(assetName: "GradientBottom", author: "John Doe"),
]
...
struct SourceHeroEffectScrollView<Content: View, Data: RandomAccessCollection>: View
where Data.Element: Identifiable {
    ...
    var body: some View {
        ...
        /// Only enable when there is no detail displaying
        .allowsHitTesting(config.sourceIndex == nil)
    }
}

struct DetailHeroEffectScrollView<Content: View, Data: RandomAccessCollection>: View
where Data.Element: Identifiable {
    @Binding var config: ScrollHeroEffectConfig
    ...
    var body: some View {
        GeometryReader {
            ...
            if let sourceIndex = config.sourceIndex, config.expandDetailView {
                /// Calculating Initial Anchor
                let anchorX: CGFloat =
                    (CGFloat(sourceIndex) * size.width) / ((CGFloat(data.count - 1) * size.width))
                ScrollView(.horizontal) {
                    HStack(spacing: config.spacing) {
                        ForEach(data, id: id) { item in
                            Rectangle()
        ...
        /// Only enable when there is detail displaying
        .allowsHitTesting(config.sourceIndex != nil)
    }
}

#Preview {
    ScrollHeroContentView()
}
